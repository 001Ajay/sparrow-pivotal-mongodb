...
public class SpringApplicationContextInitializer 
	...
	private static final Log logger = LogFactory
			.getLog(SpringApplicationContextInitializer.class);
	...
	private String[] getCloudProfile(Cloud cloud) {
		...
		logger.info("Found serviceInfos: " + StringUtils
				.collectionToCommaDelimitedString(serviceInfos));
	...
	private String[] createProfileNames(String baseName, String suffix) {
		...
		logger.info("Setting profile names: " + StringUtils
				.arrayToCommaDelimitedString(profileNames));
		return profileNames;
	}
	...
	private String[] getActiveProfile(ConfigurableEnvironment appEnvironment) {
		...
		if (serviceProfiles.size() > 1)
			throw new IllegalStateException(
					"Only one active Spring profile may be set among the following: "
